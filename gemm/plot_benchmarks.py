...
def plot(df: pd.DataFrame):
    ...
    unique_kernels = sorted(df["kernel"].unique()) # Sort to match Seaborn's default color order
    
    for i, kernel in enumerate(unique_kernels):
        # Filter using the ACTUAL kernel ID, not the loop index 'i'
        subset = df[df["kernel"] == kernel]
        
        # right align the text
        plt.text(
            subset["size"].iloc[-1], 
            subset["gflops"].iloc[-1] + 300,
            # Use .get() to prevent a crash if kernel 5 isn't in KERNEL_NAMES yet
            f"{kernel}:{KERNEL_NAMES.get(kernel, 'Unknown')}", 
            color=colors[i],
